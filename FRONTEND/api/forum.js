...
export default async function handler(req, res) {
    ...
    try {
        if (action === 'award-xp' && req.method === 'POST') {
            return await handleAwardXP(req, res);
        } else if (action === 'get-user-rank' && req.method === 'GET') {
            return await handleGetRank(req, res);
        } else if (action === 'toggle-like' && req.method === 'POST') {
            return await handleToggleLike(req, res);
        } else {
...
async function handleAwardXP(req, res) {
    ...
    const credentials = Buffer.from(`${process.env.WOO_CONSUMER_KEY}:${process.env.WOO_CONSUMER_SECRET}`).toString('base64');

    const userRes = await fetch(`${WOO_URL}/wp-json/wc/v3/customers/${userId}`, {
    ...
    await fetch(`${WOO_URL}/wp-json/wc/v3/customers/${userId}`, {
        ...
        body: JSON.stringify({
            meta_data: [
                ...
                { key: '_forum_level', value: rank.level.toString() }
            ]
        })
    });

    return res.status(200).json({ success: true, xp: newXP, xpAwarded: xpAmount, rank });
}
...
async function handleToggleLike(req, res) {
    ...
    if (!type || !id || !userId || !token) return res.status(400).json({ message: 'Missing fields' });

    const WOO_URL = process.env.WOO_BASE_URL || 'https://backendescapes.com';
    const credentials = Buffer.from(`${process.env.WOO_CONSUMER_KEY}:${process.env.WOO_CONSUMER_SECRET}`).toString('base64');
    const endpoint = type === 'topic' ? `${WOO_URL}/wp-json/wp/v2/posts/${id}` : `${WOO_URL}/wp-json/wp/v2/comments/${id}`;

    const response = await fetch(endpoint, { headers: { 'Authorization': `Basic ${credentials}`, 'Content-Type': 'application/json' } });
    if (!response.ok) throw new Error(`Content fetch failed: ${response.status}`);

    const content = await response.json();
    let currentLikes = [];
    if (content.meta?._likes) {
        currentLikes = typeof content.meta._likes === 'string' ? JSON.parse(content.meta._likes) : content.meta._likes;
    }

    const isLiked = currentLikes.includes(userId);
    const newLikes = isLiked ? currentLikes.filter(uid => uid !== userId) : [...currentLikes, userId];
    ...
    if (!isLiked) {
        // Award XP logic (Simplified: calling self for consolidation)
        const host = req.headers.host;
        const protocol = host.includes('localhost') ? 'http' : 'https';
        fetch(`${protocol}://${host}/api/forum?action=award-xp`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId, actionType: 'GIVE_LIKE', token })
        }).catch(err => console.warn('[FORUM] Liker XP failed', err));
        ...
        if (authorId && authorId !== userId) {
            fetch(`${protocol}://${host}/api/forum?action=award-xp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: authorId, actionType: type === 'topic' ? 'RECEIVE_LIKE_TOPIC' : 'RECEIVE_LIKE_REPLY', token })
            }).catch(err => console.warn('[FORUM] Author XP failed', err));
        }
    }

    return res.status(200).json({ success: true, liked: !isLiked, likeCount: newLikes.length });
}
