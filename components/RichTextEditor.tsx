...
import { 
  ...
  Quote, Code, Image as ImageIcon, Smile, Paperclip, Palette 
} from 'lucide-react';
  ...
  return (
    <div className={`flex flex-col border border-zinc-700 rounded-sm overflow-hidden bg-zinc-900 ${className}`}>
      ...
      <div className="p-3 bg-zinc-900 border-t border-zinc-800 flex items-center gap-4">
        <label className="cursor-pointer flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 px-3 py-1.5 rounded-sm text-xs font-bold text-zinc-300 transition-colors">
          <Paperclip className="w-3 h-3" /> Seleccionar archivo
          <input type="file" className="hidden" onChange={(e) => {
             if(e.target.files?.[0]) {
                alert(`Archivo "${e.target.files[0].name}" seleccionado (Simulación)`);
             }
          }} />
        </label>
