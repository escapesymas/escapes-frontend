          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            {isUserMenuOpen && user && (
              <div className="absolute right-4 top-16 w-48 bg-zinc-900 border border-zinc-800 shadow-2xl rounded-sm py-2 animate-fade-in hidden md:block">
                ...
                <button onClick={() => { setIsUserMenuOpen(false); onAccountClick?.(); }} className="w-full text-left px-4 py-2 text-xs font-bold uppercase text-zinc-400 hover:bg-zinc-800 hover:text-white flex items-center gap-2"><Settings className="w-4 h-4" /> Mi Cuenta</button>
                <div className="border-t border-zinc-800 mt-2 pt-2">
                  <button onClick={onLogoutClick} className="w-full text-left px-4 py-2 text-xs font-bold uppercase text-red-500 hover:bg-zinc-800 flex items-center gap-2"><LogOut className="w-4 h-4" /> Salir</button>
                </div>
              </div>
            )}
          </div>
